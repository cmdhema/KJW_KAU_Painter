...
public class HandleStar extends Handling implements Serializable 
{

	private static final long serialVersionUID = 1L;
	private int left, top;          // start point(left,top).
	private int width, height;      // width and height.
	private Polygon star;			// Declare the shape that we are going to draw.
	private boolean FillPaint;	// Boolean for Fillpaint.


	// Constructor of this class.
	public HandleStar(int left, int top, int width, int height, Color color)
	{
	...
	public void draw(Graphics g)
	{
		...
		int xp[] = new int[]{ left+width/2, left+2*width/3, left+width, left+5*width/6, left+width, left+2*width/3, left+width/2, left+width/3, left, left+width/6, left, left+width/3};
		int yp[] = new int[]{ top, top+height/4, top+height/4, top+height/2, top+3*height/4, top+3*height/4, top+height, top+3*height/4, top+3*height/4, top+height/2, top+height/4, top+height/4};
		star = new Polygon( xp, yp, 12);		// Make a new polygon object
		g2d.draw(star);		// Draw polygon


		if (FillPaint == true)	// When the fill order comes in
			g2d.fill(star);			// Fill the Polygon
