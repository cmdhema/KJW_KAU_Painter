...
public class HandleTri extends Handling implements Serializable 
{

	private static final long serialVersionUID = 1L;
	private int left, top;          // start point(left,top).
	private int width, height;      // width and height.
	private Polygon tri;			// Declare the shape that we are going to draw.
	private boolean FillPaint;	    // Boolean for Fillpaint.

	// Constructor of this class.
	public HandleTri(int left, int top, int width, int height, Color color)
	{
	...
	public void draw(Graphics g)
	{
		...
		int xp[] = new int[]{ (left+width/2), left, (left+width) }; 
		int yp[] = new int[]{ top, (top+height), (top+height) };
		tri = new Polygon( xp, yp, 3);		// Make a new polygon object
		g2d.draw(tri);		// Draw polygon


		if (FillPaint == true)	// When the fill order comes in
			g2d.fill(tri);			// Fill the Polygon
